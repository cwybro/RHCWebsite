...
RSpec.describe "index page", type: :feature do
  before :each do
    a= Location.new(:title => "1", :description => "",:address => "Utica, NY")
    allow(a).to receive(:geocode_address).and_return({lat: 7.698744, lng: -0.593262})
    a.save

    b = Location.new(:title => "2", :description => " ", :address => "Orlando, FL")
    allow(b).to receive(:geocode_address).and_return({lat: 6.434036, lng: 16.501465})
    b.save

    c = Location.new(:title => "3", :description => " ", :address => "Edinburgh, UK")
    allow(c).to receive(:geocode_address).and_return({lat: 42.8270, lng: -75.5446})
    c.save

    d= Location.new(:title => "4", :description => " ",:address => "Seattle, WA")
